...
use storage_proofs_core::parameter_cache::{
    parameter_cache_dir, parameter_cache_dir_name, ParameterMap, VERIFYING_KEY_EXT, GROTH_PARAMETER_EXT,
};
use tar::Archive;
...
pub fn download_sector_size(sector_sizes:Option<Vec<u64>>) {
    ...
    if let Some(ref sector_sizes) = sector_sizes {
        filenames.retain(|filename| {
            let remove =   has_extension(filename, VERIFYING_KEY_EXT)||( has_extension(filename, GROTH_PARAMETER_EXT)
